...
% \paragraph{乘法运算的定点误差分析}
% 定点数乘法会引入误差，主要来源于输入量化和结果量化两个阶段。为了分析这些误差，我们首先回顾定点数乘法的计算过程。对于实数 $R_1$ 和 $R_2$，它们首先被量化为定点数 $D_1 = R_1 \cdot F$ 和 $D_2 = R_2 \cdot F$，其中 $F$ 是比例因子（对于 Q0.10 格式，$F=2^{10}$）。定点数乘法的计算过程可以表示为：
% \begin{equation}
% D_1 \times D_2 = (R_1 \cdot F) \times (R_2 \cdot F) = (R_1 \times R_2) \cdot F^2
% \end{equation}

% 由于输出需要保持 Q0.10 格式，需要进行右移操作，即除以 $F$ 并向下取整：
% \begin{equation}
% (D_1 \times D_2) \gg 10 = \lfloor \frac{D_1 \times D_2}{F} \rfloor = \lfloor (R_1 \times R_2) \cdot F \rfloor
% \end{equation}

% 输入量化误差分析： 假设量化过程引入了误差，使得 $R_1 = \frac{D_1}{F} + \epsilon_1$ 和 $R_2 = \frac{D_2}{F} + \epsilon_2$，其中 $|\epsilon_1|, |\epsilon_2| \leq \frac{1}{F}$ 代表量化误差的界限。将带有误差的定点数表示代入乘法运算，得到：
% \[
% \begin{aligned}
% R_1 \times R_2 &= (\frac{D_1}{F} + \epsilon_1)(\frac{D_2}{F} + \epsilon_2) \\
% &= \frac{D_1 \times D_2}{F^2} + \frac{D_1 \times \epsilon_2}{F} + \frac{D_2 \times \epsilon_1}{F} + \epsilon_1 \times \epsilon_2
...
% 公式展开后，除了理想的 $\frac{D_1 \times D_2}{F^2}$ 项，还包含了由于输入量化误差产生的额外项。考虑到 $|D_1|, |D_2| \leq F$ 以及 $|\epsilon_1|, |\epsilon_2| \leq \frac{1}{F}$，输入误差传播的上界可以估计为：
% \begin{equation}
% \begin{aligned}
% |\frac{D_1 \times \epsilon_2}{F} + \frac{D_2 \times \epsilon_1}{F} + \epsilon_1 \times \epsilon_2| &\leq \frac{|D_1| |\epsilon_2|}{F} + \frac{|D_2| |\epsilon_1|}{F} + |\epsilon_1| |\epsilon_2| \\
% &\leq \frac{F \times (1/F)}{F} + \frac{F \times (1/F)}{F} + (\frac{1}{F}) \times (\frac{1}{F}) \\
...
% 当 $F = 2^{10}$ 时，$\frac{1}{F^2}$ 项非常小可以忽略，因此输入量化误差传播的上界近似为 $\frac{2}{F}$。

% 结果量化误差分析： 乘法结果右移操作，即公式 (2) 中的 $\lfloor \frac{D_1 \times D_2}{F} \rfloor$，会引入舍入误差，其误差上界为 $\frac{1}{F}$。

% 总误差分析： 综合输入量化误差和结果量化误差，定点数乘法运算的总误差上界 $\epsilon_{mul}$ 为两者之和：
% \begin{equation}
% |\epsilon_{mul}| \leq (\frac{2}{F} + \frac{1}{F^2}) + \frac{1}{F} = \frac{3}{F} + \frac{1}{F^2} \approx \frac{3}{F} = \frac{3}{2^{10}} \approx 0.002930
% \end{equation}
